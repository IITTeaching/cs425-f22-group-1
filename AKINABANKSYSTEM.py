...
def manager_session():
    ...
    if employee_type[0] == 'manager':
        ...
        Button(account_dashboard, text="Deposit", font=('Times New Roman',b),width=30,command=toDeposit).grid(row=5,sticky=N,padx=10)
        Button(account_dashboard, text="Withdraw", font=('Times New Roman',b),width=30,command=toWithdraw).grid(row=6,sticky=N,padx=10)
        Button(account_dashboard, text="Transfer", font=('Times New Roman',b),width=30,command=toTransfer).grid(row=7,sticky=N,padx=10)
...
def teller_session():
    ...
    if employee_type[0] == 'teller':
        ...
        #Buttons
        Button(account_dashboard, text="Personal Details", font=('Times New Roman',b),width=30,command=personal_details).grid(row=2,sticky=N,padx=10)
        Button(account_dashboard, text="Deposit", font=('Times New Roman',b),width=30,command=toDeposit).grid(row=3,sticky=N,padx=10)
        Button(account_dashboard, text="Withdraw", font=('Times New Roman',b),width=30,command=toWithdraw).grid(row=4,sticky=N,padx=10)
        Button(account_dashboard, text="Transfer", font=('Times New Roman',b),width=30,command=toTransfer).grid(row=5,sticky=N,padx=10)
...
def customer_session():
    ...
    Button(account_dashboard, text="Withdraw", font=('Times New Roman',b),width=30,command=toWithdraw).grid(row=5,sticky=N,padx=10)
    Button(account_dashboard, text="Transfer", font=('Times New Roman',b),width=30,command=toTransfer).grid(row=6,sticky=N,padx=10)
...
def toDeposit():
    ...
    Label(toDeposit_screen,text="Please SSN of Account to Deposit: ",font=('Times New Roman',d)).grid(row=0,sticky=N,pady=10,padx=10)
    Entry(toDeposit_screen,text=temp_SSN).grid(row=1,sticky=N,padx=5)
...
def depositCalc():
    ...
    bal = float(b[0])
    nbalstr = str(b[0])
    nbal = int(bal + deposit)
    cur.execute(f"UPDATE customer SET balance='{nbal}' WHERE SSN='{SSN}'")
    cur.execute(f"SELECT balance FROM customer WHERE SSN='{SSN}'")
    conn.commit()
    cbal = cur.fetchone()
    cbalstr = str(cbal[0])
...
def toWithdraw():
    global temp_SSN
    temp_SSN = StringVar()
    toWithdraw_screen = Toplevel(master)
    toWithdraw_screen.title('Confirmation Page')
    Label(toWithdraw_screen,text="Please SSN of Account to Withdraw: ",font=('Times New Roman',d)).grid(row=0,sticky=N,pady=10,padx=10)
    Entry(toWithdraw_screen,text=temp_SSN).grid(row=1,sticky=N,padx=5)
    Button(toWithdraw_screen,text="Enter",command=withdraw,width=15,font=('Times New Roman',b)).grid(row=3,sticky=N,pady=5)
def withdraw():
    global temp_withdraw
    temp_withdraw = StringVar()
    #Withdraw screen
    withdraw_screen = Toplevel(master)
    withdraw_screen.title('Withdraw')
    Label(withdraw_screen,text="Enter Amount to Withdraw: $",font=('Times New Roman',d)).grid(row=0,sticky=N,pady=10,padx=10)
    Entry(withdraw_screen,text=temp_withdraw).grid(row=1,sticky=N,padx=5)
    Button(withdraw_screen,text="Enter",command=withdrawCalc,width=15,font=('Times New Roman',b)).grid(row=3,sticky=N,pady=5)
...
def withdrawCalc():
    ...
    global SSN
    SSN = temp_SSN.get()
    withdraw = float(temp_withdraw.get())
    withdrawstr = str(temp_withdraw.get())
    withdrawCalc_screen = Toplevel(master)
    withdrawCalc_screen.title('Withdraw')
    cur.execute(f"SELECT balance FROM customer WHERE SSN='{SSN}'")
    conn.commit()
    b = cur.fetchone()
    bal = float(b[0])
    nbalstr = str(b[0])
    nbal = int(bal - withdraw)
    cur.execute(f"UPDATE customer SET balance='{nbal}' WHERE SSN='{SSN}'")
    cur.execute(f"SELECT balance FROM customer WHERE SSN='{SSN}'")
    conn.commit()
    cbal = cur.fetchone()
    cbalstr = str(cbal[0])
    Label(withdrawCalc_screen,text="The Following Withdrawal was Made: $" + withdrawstr,font=('Times New Roman',d)).grid(row=0,sticky=N,pady=10,padx=10)
    Label(withdrawCalc_screen,text="Balance Before Deposit: $" + nbalstr,font=('Times New Roman',d)).grid(row=1,stick=N)
    Label(withdrawCalc_screen,text="Current Balance: $" + cbalstr,font=('Times New Roman',d)).grid(row=2,sticky=N,pady=10,padx=10)
    Button(withdrawCalc_screen,text="Return to Account Management",command=account_management,width=30,font=('Times New Roman',b)).grid(row=3,sticky=N,pady=5)
...
def transfer():
    ...
    Label(transfer_screen,text="Enter SSN of Account to Transfer Money to",font=('Times New Roman',d)).grid(row=0,sticky=N,pady=10,padx=10)
    Entry(transfer_screen,text=temp_SSN).grid(row=1,sticky=N,padx=5)
    Label(transfer_screen,text="Enter Transfer Amount",font=('Times New Roman',d)).grid(row=3,sticky=N,pady=10,padx=10)
    Entry(transfer_screen,text=temp_trans).grid(row=4,sticky=N,padx=5)
    Button(transfer_screen,text="Enter",command=transferCalc,width=15,font=('Times New Roman',b)).grid(row=5,sticky=N,pady=5)
def transferCalc():
    global trans
    global transstr
    global SSN
    ...
    cur.execute(f"SELECT balance FROM customer WHERE SSN='{ownSSN}'")
    conn.commit()
    ownb = cur.fetchone()
    ownbal = float(ownb[0])
    ownbalstr = str(ownb[0])
    nownbal = int(ownbal - trans)
    cur.execute(f"UPDATE customer SET balance='{nownbal}' WHERE SSN='{ownSSN}'")
    cur.execute(f"SELECT balance FROM customer WHERE SSN='{ownSSN}'")
    conn.commit()
    ...
    Button(transferCalc_screen,text="Return to Account Management",comand=account_management,width=30,font=('Times New Roman',b)).grid(row=5,sticky=N,pady=5)
