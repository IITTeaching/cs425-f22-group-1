...
def finish_account_reg():
    ...
    if SSN_account_input == "" or account_type_input == "" :
        ...
        cur.execute(f"INSERT INTO  account3 VALUES(default,'{account_type_input}', 0, '{SSN_account_input}')")
        conn.commit()
    
        login_notif = Label(account_register_screen, font=('Times New Roman',n))
        login_notif.grid(row=5,sticky=N)
        login_notif.config(fg="green",text="New account added!")
...
def delete_account():
    ...
    #Buttons
    Button(account_register_screen, text="Show accounts", command = finish_account_reg, font=('Times New Roman',b)).grid(row=14,sticky=N,pady=10)
...
def show_account:
    ...
    if account != []:
        ...
        for i in len(account):
            first_account = account[i]
            account_num = first_account[0]
...
def delete_account_cont():
    ...
    SSN_account_del_input = temp_account_SSN.get()

    if SSN_account_del_input == "":
        login_notif = Label(account_register_screen, font=('Times New Roman',n))
        login_notif.grid(row=5,sticky=N)
        login_notif.config(fg="red",text="All fields required * ")
        return
        ...
        cur.execute(f"select * from account3 where customerSSN = '{SSN_account_del_input}'")
        conn.commit()
...
def deposit():
    cur.execute(f"select * from account where account.customerSSN = '{ssn_cust}'")
...
cur.execute(f"SELECT account_num FROM account3 WHERE customerSSN='123456789'")
conn.commit()
account = cur.fetchall()
print(account)
len(account)
