...
def finish_reg():
    ...
    if SSN_input == "" or Fname == "" or Lname == "" or street_number == "" or street_name == "" or city == "" or state == "" or zipp == "" or home_branch == "" or role == "" or password == "":
...
def register():
    ...
    Label(register_screen, text="Role", font=('Times New Roman',a)).grid(row=12,sticky=W)
    Label(register_screen, text="Password", font=('Times New Roman',a)).grid(row=13,sticky=W)
    notif = Label(register_screen, font=('Times New Roman',n))
    notif.grid(row=14,sticky=N,pady=10)
    ...
    Entry(register_screen,textvariable=temp_role).grid(row=12,column=1)
    Entry(register_screen,textvariable=temp_password,show="*").grid(row=13,column=1)
    
    #Buttons
    Button(register_screen, text="Register", command = finish_reg, font=('Times New Roman',b)).grid(row=15,sticky=N,pady=10)
...
def create_account(customerID):
    ...
    for branch in records:
        branch_dictionary[branch[1]] = branch[0]
        print(f"id:{branch[1]} address:{branch[0]}")
    branch_id = int(input("Enter branch ID\n"))
    print("Select account type")
    for x in range(len(account_type)):
        print(f"{x}: {account_type[x]}")
    acc_type = int(input("Enter account type\n"))
    balance = float(input("Enter balance\n"))
    cur.execute(f"insert into accounts values('{str(uuid.uuid4())}', '{account_type[acc_type]}', '{balance}', '{customerID}', '{branch_dictionary[branch_id]}');")

def delete_account(customerID):
    account_num = int(input("\nPlease enter your account number to continue: "))
    print("\nEnsure that you withdraw or transfer any remaining funds in the account before proceeding\n")
    sql = f"SELECT a.account_id, a.account_num, t.account_type FROM customer c LEFT JOIN account a ON c.customerID = a.account_id LEFT JOIN accounttype t ON a.account_id = t.account_id WHERE a.account_num = {account_num};"
    cur.execute(sql)
    accounts = cur.fetchall()
    account_ID1 = []
    for rows in accounts:
        account_ID1.append(int(rows[0]))
        print(f"{rows[0]} - Account #{rows[1]} & Account Type: {rows[2]}")
    choice = 0
    while choice not in account_ID1:
    ...
    if balance[0] == 0:
        ...
        print("\nPlease remove any remaining funds in account.")
        pass
    # Executes deletion
    try:
...
def deposit(customerID):
    cur.execute(f"select * from accounts where customer_id = '{customerID}';")
    ...
    account_ChoiceID = int(input("Please select account for deposit :"))
    ...
    records = cur.fetchall()
    print(f"Current Balance : {records[0][2]}")
    account_Balance = records[0][2] + deposit_amount
    cur.execute(f"update accounts set balance = {account_Balance} where account_id = '{userAccountsDictionary[account_ChoiceID]}';")
    print("Transaction Successful")
    print(f"Current Balance : {account_Balance}")
    cur.execute(f"insert into transaction values('{records[0][0]}', 'Self', 'Deposit', {deposit_amount}, 'Debit Cash', current_timestamp);")
